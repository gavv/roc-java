...
public class RocReceiverConfig {
     ...
     * latency. Then, if clock synchronization is enabled, the session will
     * adjust its clock to keep actual latency as close as possible to the
     * target latency. If zero, default value is used.
     */
     ...
     * <p>
     * If session latency differs from the target latency by more than given
     * value, the session is terminated (it can then automatically restart).
     * Receiver itself is not terminated; if there are no sessions, it will
     * produce zeros. If zero, default value is used.
     */
     ...
     * can then automatically restart). Receiver itself is not terminated; if
     * there are no sessions, it will produce zeros. This mechanism allows to
     * detect dead, hanging, or incompatible clients that generate unparseable
     * packets. If zero, default value is used. If negative, the timeout is
     * disabled.
     */
     ...
     * terminated; if there are no sessions, it will produce zeros. This
     * mechanism allows to detect situations when playback continues but there
