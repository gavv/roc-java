 ...
 * <p>
 * Each slot of a peer (see {@link Slot} ) has multiple interfaces, one for each
 * of the interface types. The user interconnects peers by binding an interface
 * of one peer to an URI, and then connecting the corresponding interface of
 * another peer to that URI.
 * <p>
 ...
 * {@link Interface#AUDIO_SOURCE}, {@link Interface#AUDIO_REPAIR}, and
 * {@link Interface#AUDIO_CONTROL}.
 * <p>
 * {@link Interface#CONSOLIDATED} is mutually exclusive with lower-level
 * interfaces. In most cases, the user needs only
 * {@link Interface#CONSOLIDATED}. However, the lower-level interfaces may be
 * useful if an external signaling mechanism is used or for compatibility with
 ...
 * <p>
 * {@link Interface#AUDIO_CONTROL} is a lower-level bidirectional interface for
 * control streams. If you use {@link Interface#AUDIO_SOURCE} and
 * {@link Interface#AUDIO_REPAIR}, you usually also need to use
 * {@link Interface#AUDIO_CONTROL} to enable carrying additional non-transport
 * information.
 */
